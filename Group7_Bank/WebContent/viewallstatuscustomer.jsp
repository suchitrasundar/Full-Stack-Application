...
<div id="centeralign" style="height:600px;">
...
<%
	ArrayList<Customer> allList = (ArrayList<Customer>) request.getAttribute("allcust");
%>
...
<th>Refresh</th>
		...
		<%
			Iterator myIterator = allList.iterator();
			while (myIterator.hasNext()) {
				Customer allcust = (Customer) myIterator.next();
		%>
		
			<tr>
				<td><%=allcust.getCustId()%></td>
				<td><%=allcust.getCustSSN()%></td>
				<td><%=allcust.getCustName() %></td>
				<td><%=allcust.getCustAddress()%></td>
				<td><%=allcust.getCustState()%></td>
				<td><%=allcust.getCustCity()%></td>
				<td><a href="searchAllCustomerDetails.html">refresh</a></td>
			</tr>
